...
-------------------------------------------------------------------------------
    ...
    Copyright (C) 2023 Ganeshkumar (IITGN)
-------------------------------------------------------------------------------
License
...
)
{
    ...
    word gasFieldName(fieldValueStream);
    ...
    if (fieldHeader.typeHeaderOk<fieldType>(true))
    {
        ...
        if (!field.write())
        {
            FatalErrorInFunction
              << "Failed writing field " << gasFieldName << endl;
        }
    }
    else
    {
        WarningInFunction
          << "Field " << gasFieldName << " not found" << endl;
...
int main(int argc, char *argv[])
{
    ...
    const word dictName("setRocketInitFieldDict");
    #include "setSystemMeshDictionaryIO.H"

    IOdictionary setRocketInitFieldDict(dictIO);

    Info<< "Reading " << setRocketInitFieldDict.name() << endl;

    const word gasFieldName = setRocketInitFieldDict.get<word>("gasField");
    const bool writeOBJ = setRocketInitFieldDict.getOrDefault("writeOBJ", false);
    const word propellantFieldName = setRocketInitFieldDict.get<word>("propellantField");

    Info<< "Reading field " << gasFieldName << nl << endl;
    volScalarField alpha1
    (
        IOobject
        (
    ...
    (
        setRocketInitFieldDict.get<word>("type"),
        setRocketInitFieldDict
    );
    ...
    if (writeOBJ)
    {
        isoFacesToFile(facePts, gasFieldName + "0");
    }

    ISstream::defaultPrecision(18);

    Info<< "Writing gas phase alpha field " << alpha1.name() << endl;
    alpha1.write();

    Info<< "Creating propellant phase field " << propellantFieldName << nl << endl;
    ...
    // Set post combustion chamber and Nozzle field
    PtrList<entry> regions(setRocketInitFieldDict.lookup("regions"));
    ...
    regressionAlpha.write();
    
    // Estimate Propellant Volume
    const scalarField& alpha = alphaProp.internalField();

    Info << nl << "Total Propellant Volume: " << gSum(mesh.V()*alpha) << " m^3 " << endl;
